        ...
        <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all group flex flex-col h-full">
            <div class="h-52 bg-gray-200 overflow-hidden relative">
                @if((!empty($post->images) ? $post->images[0] : null))
                    <img src="{{ Str::startsWith((!empty($post->images) ? $post->images[0] : null), 'http') ? (!empty($post->images) ? $post->images[0] : null) : asset('storage/'.(!empty($post->images) ? $post->images[0] : null)) }}" alt="{{ $post->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                @else
