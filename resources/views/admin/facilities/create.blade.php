<x-admin-layout title="Tambah Fasilitas">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.facilities.store') }}" method="POST" class="p-6 sm:p-8 space-y-5">
            ...
            @endphp
            <div x-data="{ selectedIcon: '{{ old('icon', 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6') }}' }">
                    ...
                    <div class="flex-grow">
                        <select name="icon" x-model="selectedIcon" required
                            ...
                            @foreach($icons as $path => $label)
                                <option value="{{ $path }}" {{ old('icon') == $path ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
