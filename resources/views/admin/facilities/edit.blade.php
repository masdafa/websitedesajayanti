<x-admin-layout title="Edit Fasilitas">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.facilities.update', $facility) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-5">
            @csrf @method('PUT')

            {{-- Multiple Image Upload --}}
            <div x-data="{ 
                initialPreviews: {{ json_encode(collect($facility->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                previews: {{ json_encode(collect($facility->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                hasNewFiles: false,
                handleFileChange(e) {
