        ...
        <form action="{{ route('admin.facilities.update', $facility) }}" method="POST" class="p-6 sm:p-8 space-y-5">
            ...
            <div>
                ...
                <textarea name="icon" rows="3" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3 font-mono">{{ old('icon', $facility->icon) }}</textarea>
                <p class="mt-1 text-xs text-gray-500">Nilai atribut <code>d="..."</code> dari <code>&lt;path&gt;</code> SVG Heroicons. Lihat <a href="https://heroicons.com" target="_blank" class="text-emerald-600 underline">heroicons.com</a>.</p>
