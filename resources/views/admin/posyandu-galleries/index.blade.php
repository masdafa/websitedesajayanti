<x-admin-layout title="Dokumentasi Posyandu">
    <x-slot:breadcrumb>Kelola foto-foto dokumentasi kegiatan Posyandu</x-slot:breadcrumb>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="p-4 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row gap-4 items-center justify-between">
            <h2 class="font-bold text-gray-800">Daftar Dokumentasi Posyandu</h2>
            
            <a href="{{ route('admin.posyandu-galleries.create') }}" class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-semibold py-2.5 px-5 rounded-xl transition flex items-center justify-center gap-2">
        ...
        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 bg-gray-50/50">
            @forelse($galleries as $gallery)
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition group">
                    <div class="aspect-w-4 aspect-h-3 bg-gray-100 relative">
                        @if($gallery->image)
                            <img src="{{ Storage::url($gallery->image) }}" class="w-full h-48 object-cover">
                        @else
                        ...
                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3 backdrop-blur-sm">
                            <a href="{{ route('admin.posyandu-galleries.edit', $gallery) }}" class="bg-white text-blue-600 p-2 rounded-lg hover:scale-110 transition shadow-lg" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                            </a>
                            <form action="{{ route('admin.posyandu-galleries.destroy', $gallery) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto ini?');">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <div class="col-span-full py-12 text-center text-gray-500 bg-white rounded-2xl border border-gray-200">
                    Belum ada dokumentasi Posyandu.
                </div>
            @endforelse
        </div>
    </div>
