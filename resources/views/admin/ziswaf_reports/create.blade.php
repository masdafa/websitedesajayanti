<x-admin-layout title="Tambah Laporan ZISWAF">
    <x-slot:breadcrumb>Tambah Laporan ZISWAF</x-slot:breadcrumb>

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Laporan ZISWAF</h1>
    </div>

    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.ziswaf-reports.store') }}" method="POST" class="p-6 sm:p-8 space-y-5">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">Bulan / Periode <span class="text-red-500">*</span></label>
                <input type="text" name="month_name" value="{{ old('month_name') }}" required
