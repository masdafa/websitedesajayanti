<x-admin-layout title="Edit PHBI">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.phbi-events.update', $phbiEvent) }}" method="POST" class="p-6 sm:p-8 space-y-5">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">Ikon</label>
                <input type="text" name="icon" value="{{ old('icon', $phbiEvent->icon) }}"
