<x-admin-layout title="Edit PHBI">
    <x-slot:breadcrumb>Perbarui data PHBI warga</x-slot:breadcrumb>
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.phbi-events.update', $phbiEvent) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-5">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">Nama PHBI <span class="text-red-500">*</span></label>
                <input type="text" name="title" value="{{ old('title', $phbiEvent->title) }}" required
            ...
            <input type="hidden" name="icon" value="{{ old('icon', $phbiEvent->icon) }}">

            {{-- Multiple Image Upload --}}
            <div x-data="{ 
                initialPreviews: {{ json_encode(collect($phbiEvent->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                previews: {{ json_encode(collect($phbiEvent->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                hasNewFiles: false,
                handleFileChange(e) {
...
<div>
                ...
                <textarea name="description" rows="3"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3">{{ old('description', $phbiEvent->description) }}</textarea>
