<x-admin-layout title="Profil Karang Taruna">
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50/50">
            <h2 class="text-lg font-bold text-gray-900">Edit Profil Karang Taruna</h2>
            <p class="text-sm text-gray-500">Sesuaikan informasi profil Karang Taruna untuk ditampilkan pada halaman warga.</p>
        </div>

        <form action="{{ route('admin.karang-taruna-profile.update') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Isi Profil (Deskripsi, Visi, Misi, Sejarah, dll)</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Gambar/Foto Representasi (Opsional)</label>
                @if($profile->image)
                    <div class="mb-3">
                        <img src="{{ Storage::url($profile->image) }}" alt="Foto Karang Taruna" class="w-48 rounded-lg shadow-sm">
                    </div>
                @endif
