<x-admin-layout title="Edit Dokumentasi DKM">
    <x-slot:breadcrumb>Ubah data dokumentasi kegiatan DKM</x-slot:breadcrumb>
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.dkm-galleries.update', $dkm_gallery) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-5">
            @csrf
            @method('PUT')
            
            <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">Judul Foto <span class="text-red-500">*</span></label>
                <input type="text" name="title" value="{{ old('title', $dkm_gallery->title) }}" required
            ...
            <div x-data="{ 
                initialPreviews: {{ json_encode(collect($dkm_gallery->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                previews: {{ json_encode(collect($dkm_gallery->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                hasNewFiles: false,
                handleFileChange(e) {
...
<div>
                ...
                <textarea name="description" rows="3"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3">{{ old('description', $dkm_gallery->description) }}</textarea>
            ...
            <div class="pt-4 flex gap-3 border-t border-gray-100">
                ...
                <a href="{{ route('admin.dkm-galleries.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold py-2.5 px-6 rounded-xl transition">Batal</a>
            </div>
        </form>
    </div>
</x-admin-layout>
