<x-admin-layout title="Edit Produk">
    <x-slot:breadcrumb>Ubah data produk Belanja Desa</x-slot:breadcrumb>
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="seller_name" value="{{ old('seller_name', $product->seller_name) }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3"
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3"
                    placeholder="Jelaskan detail produk ini...">{{ old('description', $product->description) }}</textarea>
            ...
            <div x-data="{ 
                initialPreviews: {{ json_encode(collect($product->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                previews: {{ json_encode(collect($product->images ?? [])->map(fn($img) => asset('storage/'.$img))->toArray()) }},
                hasNewFiles: false,
                handleFileChange(e) {
