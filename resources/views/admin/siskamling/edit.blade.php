<x-admin-layout title="Edit Jadwal Siskamling">
    <x-slot:breadcrumb>Perbarui jadwal ronda malam</x-slot:breadcrumb>
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.siskamling.update', $siskamling) }}" method="POST" class="p-6 sm:p-8 space-y-5">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">Hari <span class="text-red-500">*</span></label>
                <input type="text" name="day" value="{{ old('day', $siskamling->day) }}" required
            ...
            <div>
                ...
                <textarea name="members" rows="3" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3">{{ old('members', $siskamling->members) }}</textarea>
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $siskamling->sort_order) }}" min="0"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full px-4 py-3">
            </div>
